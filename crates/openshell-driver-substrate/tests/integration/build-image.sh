...
echo "[build-image] cargo build --release -p openshell-driver-substrate --bin openshell-sandbox-substrate" >&2
( cd "$WORKSPACE_ROOT" && cargo build --release -p openshell-driver-substrate --bin openshell-sandbox-substrate ) >&2
...
cp "$WORKSPACE_ROOT/crates/openshell-sandbox/data/sandbox-policy.rego" "$BUILD_CTX/policy.rego"
