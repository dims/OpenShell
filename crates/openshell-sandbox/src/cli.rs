...
//! `OpenShell` Sandbox CLI entry point.
//!
//! Extracted from `main.rs` so that alternative supervisor binaries (e.g.
//! ones that register a non-default [`crate::SandboxFailureHandler`] for
//! outer-sandbox-managed degraded mode) can reuse the same CLI surface
//! without duplicating arg parsing or runtime setup.
...
/// CLI entry point. Parses args, sets up logging + runtime, then invokes
/// [`crate::run_sandbox`].
///
/// Binaries that need a non-default [`crate::SandboxFailureHandler`] should
/// call [`crate::set_failure_handler`] before invoking this function.
pub fn run() -> Result<()> {
    ...
    if raw_args.get(1).map(String::as_str) == Some(DEBUG_RPC_SUBCOMMAND) {
        ...
        return runtime.block_on(async move {
            ...
            let exit = crate::debug_rpc::run(&raw_args[2..]).await?;
    ...
    let exit_code = runtime.block_on(async move {
        ...
        {
            let (tx, handle) = crate::log_push::spawn_log_push_task(
                endpoint.clone(),
                sandbox_id.clone(),
            );
            let layer = crate::log_push::LogPushLayer::new(sandbox_id.clone(), tx);
            Some((layer, handle))
        } else {
            None
        };
